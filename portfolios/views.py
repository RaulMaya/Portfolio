...
projects_done = {
    ...
    'earthquakes':{
        'description':'The main objective of this project is to show the user, in a simple and direct way, the last week earthquakes around the world, and the location of the tectonic plates. For a better understanding, each earthquake has its own visual characteristic, such as, the bigger the radius of the marker is the higher the magnitude of that earthquake was, and the darker the marker is, the deeper that earthquake was. The project was developed using Leaflets Library, and the data for the earthquakes was obtained from the U.S. Geological Survey.',
        'image':'https://www.collinsdictionary.com/images/full/earthquake_139550174.jpg'},
    'star wars game':{
        'description':'The user controls the Millenium Falcon. He can move the space ship in right and left direction using the arrow keys and shoot rockets by pressing the spacebar. When the game begins, a group of TIE-Fighters will fill the sky. The user shoots and destroys the TIE-Fighters. If the user shoots all the TIE-Fighters, a new fleet will appear, that moves faster than the last one. If any TIE-Fighter hits the player ship or reaches the bottom, the user will lose a life. If the user loses three lifes, the game will be over.',
    ...
    'citi bikes':{
        'description':'Since 2013, the Citi Bike Program has implemented a robust infrastructure for collecting data on the programs utilization. Through the teams efforts, each month bike data is collected, organized, and made public on the Citi Bike Data webpage.However, while the data has been regularly updated, the team has yet to implement a dashboard or sophisticated reporting process. City officials have a number of questions on the program, so your first task on the job is to build a set of data reports to provide the answers.',
...
def projects(request):
    ...
    projects_images = zip(projects,images)
